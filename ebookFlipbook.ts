...
<div class="app" id="app">
  ...
  <main class="main">
    ...
    <section class="stage-wrap" id="stage">
      ...
      <div class="hud"><button class="btn" id="hudPrev" type="button">‹</button><span class="chip" id="hudPage">0 / 0</span><button class="btn" id="hudNext" type="button">›</button><button class="btn hidden" id="resetZoomBtn" type="button">↩</button></div>
    </section>
...
(function(){
  ...
  function initFlip(){
    try {
      pageFlip = new St.PageFlip(book, { width: 560, height: 760, size:'stretch', minWidth:260, maxWidth:620, minHeight:360, maxHeight:820, showCover:true, usePortrait:true, mobileScrollSupport:false, maxShadowOpacity:.38, flippingTime:400, direction: cfg.direction === 'rtl' ? 'rtl' : 'ltr' });
  ...
  setText('firstBtn', txt.first); setText('prevBtn', txt.prev); setText('nextBtn', txt.next); setText('lastBtn', txt.last); setText('notesBtn', txt.notes); setText('fullBtn', txt.full); setText('downloadBtn', txt.pdf); setText('notesTitle', txt.notes); setText('searchBtn', txt.search); setText('resetZoomBtn', txt.zoomReset);
  ...
  var fsApp = document.getElementById('app'), fsBtn = document.getElementById('fullBtn');
  function fsLabel(on){ return on ? (isFa ? 'خروج' : 'Exit') : txt.full; }
  function enterFs(){
    var el = document.documentElement;
    if(el.requestFullscreen) el.requestFullscreen().catch(function(){});
    else if(el.webkitRequestFullscreen) el.webkitRequestFullscreen();
    else { fsApp.classList.add('fs-mode'); setText('fullBtn', fsLabel(true)); }
  }
  function exitFs(){
    if(document.exitFullscreen) document.exitFullscreen().catch(function(){});
    else if(document.webkitExitFullscreen) document.webkitExitFullscreen();
    else { fsApp.classList.remove('fs-mode'); setText('fullBtn', fsLabel(false)); }
  }
  fsBtn.onclick = function(){ if(document.fullscreenElement||document.webkitFullscreenElement||fsApp.classList.contains('fs-mode')) exitFs(); else enterFs(); };
  document.addEventListener('fullscreenchange', function(){ setText('fullBtn', fsLabel(!!document.fullscreenElement)); });
  document.addEventListener('webkitfullscreenchange', function(){ setText('fullBtn', fsLabel(!!document.webkitFullscreenElement)); });
  ...
  var dragOvEl = document.getElementById('dragOverlay');
  var az = {scale:1,tx:0,ty:0,dragging:false,sx:0,sy:0,ptx:0,pty:0,pinchDist:0,pinchScale:1};
  function applyAz(){
    zoomWrapEl.style.transform = 'translate('+az.tx+'px,'+az.ty+'px) scale('+az.scale+')';
    var zoomed = az.scale > 1.05;
    stageEl.classList.toggle('zoomed', zoomed);
    document.getElementById('resetZoomBtn').classList.toggle('hidden', !zoomed);
  }
  function resetAz(){ az.scale=1; az.tx=0; az.ty=0; applyAz(); }
  stageEl.addEventListener('wheel', function(e){
    e.preventDefault();
    var rect = stageEl.getBoundingClientRect();
    var px = e.clientX - rect.left, py = e.clientY - rect.top;
    var factor = e.deltaY < 0 ? 1.15 : (1/1.15);
    var ns = Math.max(1, Math.min(6, az.scale * factor));
    if(Math.abs(ns - az.scale) < 0.001) return;
    az.tx = px - (ns/az.scale)*(px - az.tx);
    az.ty = py - (ns/az.scale)*(py - az.ty);
    az.scale = ns; applyAz();
  }, {passive:false});
  dragOvEl.addEventListener('mousedown', function(e){
    az.dragging=true; az.sx=e.clientX; az.sy=e.clientY; az.ptx=az.tx; az.pty=az.ty;
    dragOvEl.classList.add('panning'); e.preventDefault();
  });
  document.addEventListener('mousemove', function(e){
    if(!az.dragging) return;
    az.tx = az.ptx+(e.clientX-az.sx); az.ty = az.pty+(e.clientY-az.sy); applyAz();
  });
  document.addEventListener('mouseup', function(){
    if(!az.dragging) return;
    az.dragging=false; dragOvEl.classList.remove('panning');
  });
  stageEl.addEventListener('touchstart', function(e){
    if(e.touches.length===2){
      az.pinchDist=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);
      az.pinchScale=az.scale; e.preventDefault();
    } else if(e.touches.length===1 && az.scale>1.05){
      az.dragging=true; az.sx=e.touches[0].clientX; az.sy=e.touches[0].clientY; az.ptx=az.tx; az.pty=az.ty;
      e.preventDefault();
    }
  },{passive:false});
  stageEl.addEventListener('touchmove', function(e){
    if(e.touches.length===2){
      ...
      var ns=Math.max(1,Math.min(6,az.pinchScale*(d/az.pinchDist)));
      var cx=(e.touches[0].clientX+e.touches[1].clientX)/2;
      var cy=(e.touches[0].clientY+e.touches[1].clientY)/2;
      var rect=stageEl.getBoundingClientRect();
      var px=cx-rect.left, py=cy-rect.top;
      az.tx=px-(ns/az.scale)*(px-az.tx); az.ty=py-(ns/az.scale)*(py-az.ty);
      az.scale=ns; applyAz(); e.preventDefault();
    } else if(e.touches.length===1 && az.dragging){
      az.tx=az.ptx+(e.touches[0].clientX-az.sx); az.ty=az.pty+(e.touches[0].clientY-az.sy);
      applyAz(); e.preventDefault();
    }
  },{passive:false});
  stageEl.addEventListener('touchend', function(){ az.dragging=false; dragOvEl.classList.remove('panning'); });
  document.getElementById('resetZoomBtn').onclick = resetAz;
