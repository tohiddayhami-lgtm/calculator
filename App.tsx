          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                { label: 'Total Profit (FOB)', value: formatMoney(grandProfit, config.outputCurrency), sub: `${margin.toFixed(1)}% margin`, color: 'bg-gradient-to-br from-emerald-500 to-emerald-700', icon: <PieChart className="w-5 h-5 opacity-80" /> },
                ...
                {grandTotalCost > 0 ? (
                  <div className="space-y-4">
                        ...
                        return (
                          <div key={i} className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-sm font-semibold text-slate-700 w-28 text-right">{formatMoney(seg.value, config.outputCurrency)}</span>
                            </div>
                          </div>
                        );
                      })}
                    ...
                    <div className="pt-3 border-t border-slate-100 flex justify-between">
                      ...
                      <span className="font-bold text-slate-800">{formatMoney(grandTotalCost, config.outputCurrency)}</span>
                    </div>
                  </div>
                ) : (
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex bg-slate-100 rounded-lg p-0.5">
                    {(['20ft', '40ft'] as const).map(ct => (
                      <button key={ct} onClick={() => setContainerType(ct)} className={`px-2.5 py-1 text-xs font-medium rounded-md transition-all ${containerType === ct ? 'bg-white text-cyan-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>{ct}</button>
                    ))}
                  </div>
                </div>
                <div className="flex items-center gap-2 mb-4">
                  <label className="text-xs text-slate-500 whitespace-nowrap">Units per {containerType}:</label>
                  <input
                    type="number"
                    value={containerCapacity}
                    onChange={(e) => setContainerCapacity(Math.max(1, parseInt(e.target.value) || 1))}
                    className="w-24 text-sm border border-slate-200 rounded px-2 py-1 text-center focus:ring-1 focus:ring-cyan-500 outline-none"
                  />
                  <span className="text-xs text-slate-400 ml-1">Total: {calculations.totalQty.toLocaleString()} units</span>
                </div>
                {/* Container Box */}
                <div className="relative w-full h-28 rounded-md overflow-hidden border-[3px] border-slate-500 bg-slate-100" style={{ boxShadow: 'inset 0 2px 8px rgba(0,0,0,0.12)' }}>
                  {/* Corrugated wall lines */}
                  {[...Array(9)].map((_, i) => (
                    <div key={i} className="absolute top-0 bottom-0 border-r border-slate-300/50" style={{ left: `${(i + 1) * 10}%` }} />
                  ))}
                  {/* Product fill segments */}
                  <div className="absolute bottom-0 left-0 top-0 flex">
                    {productFills.map((pf) => (
                      <div
                        key={pf.id}
                        style={{ width: `${pf.pct}%`, background: pf.color, opacity: 0.82, minWidth: pf.pct > 0 ? '2px' : 0 }}
                        title={`${pf.name}: ${pf.qty.toLocaleString()} units`}
                  ...
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="bg-white/85 backdrop-blur-sm rounded-lg px-3 py-1.5 text-center shadow-md">
                      ...
                      <div className="text-[10px] text-slate-500">{containersNeeded > 1 ? `last of ${containersNeeded} containers` : `of ${containerType}`}</div>
                    </div>
                  </div>
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                  <div className="bg-cyan-50 border border-cyan-100 rounded-lg p-2">
                    ...
                    <div className="text-[10px] text-cyan-500">Containers needed</div>
                  </div>
                  <div className="bg-slate-50 border border-slate-100 rounded-lg p-2">
                    <div className="text-lg font-bold text-slate-700">{containerCapacity.toLocaleString()}</div>
                    <div className="text-[10px] text-slate-400">Capacity / container</div>
                  </div>
                  <div className="bg-slate-50 border border-slate-100 rounded-lg p-2">
                    <div className="text-lg font-bold text-slate-700">{remainingInLast.toLocaleString()}</div>
                    <div className="text-[10px] text-slate-400">Remaining space</div>
                  </div>
                </div>
                {/* Product legend */}
                {productFills.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {productFills.map(pf => (
                      <div key={pf.id} className="flex items-center gap-1 bg-slate-50 rounded-full px-2 py-0.5">
                        ...
                        <span className="text-[10px] text-slate-400">{pf.qty.toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                )}
